function bool = bpmMio(u, seen, edmondsMtx,G,minimumD)
%BPM A DFS based recursive function that returns true if a matching for
% non-leftover u is possible
...
%             totWeight = 
            ...
            if matchR(v) == -1 || bpmMio(matchR(v),seen, edmondsMtx,G,minimuD)                
                matchR(v) = u;
                kRootedTrees = constructTrees(G,matchR, minimumD);
                bool = true;
%                 return;
            end
        end
